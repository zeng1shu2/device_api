...
class Fw_Access(object):
    ...
    def get_text(self):
        """请求查询接口"""
        result_get = Fw_Access.s.get(
            self.url, headers=self.headers, verify=False)
        return result_get

    def post_text(self, j_data):
        """新增信息"""
        response = Fw_Access.s.post(
            self.url, data=j_data, headers=self.headers, verify=False)
        return response

    def put_text(self, j_data):
        """修改系统"""
        response = Fw_Access.s.put(
            self.url, headers=self.headers, data=j_data, verify=False)
        return response

    def del_text(self):
        """删除信息"""
        response = Fw_Access.s.delete(
            self.url, headers=self.headers, verify=False)
        return response
